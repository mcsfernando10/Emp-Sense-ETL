...
# Read in the data.
msg.loading_message()
df_mysql = mysql_cn.read('select * from employeesit_raw')
...
to_drop = ['Employee_ID', 'Employee_Name', 'churn']
...
counts = pandas.value_counts(np.ndarray.round(pred_churn, 1))
...
probability = np.ndarray.round(pred_churn, 1)
probability.tolist()

msg.update_message()
for num in range(0,len(id)):
    mysql_cn.insert_update("UPDATE employeesit SET probability=%s WHERE Employee_ID='%s'" %
